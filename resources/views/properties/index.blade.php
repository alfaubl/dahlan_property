            ...
            <div class="col-lg-9">
                <div class="results-header">
                    <h5 class="fw-bold mb-0">
                        <span id="propertyCount">{{ $properties->count() }}</span> Properti Ditemukan
                    </h5>
                    ...
                    <div class="col-md-6 col-xl-4">
                        <div class="property-card card h-100">
                            <div class="property-image position-relative">
                                <span class="property-badge position-absolute top-0 start-0 m-3">{{ $property->status }}</span>
                                <span class="property-type position-absolute top-0 end-0 m-3">{{ $property->type }}</span>
                                <img src="{{ $property->image ?? 'https://images.unsplash.com/photo-1568605114967-8130f3a36994' }}" 
                                     alt="{{ $property->title }}" class="card-img-top" style="height: 200px; object-fit: cover;">
                            </div>
                            <div class="card-body d-flex flex-column">
                                <div class="property-price text-primary fw-bold mb-2">
                                    Rp {{ number_format($property->price, 0, ',', '.') }}
                                    <small class="text-muted">/{{ $property->purpose == 'sale' ? 'jual' : 'bulan' }}</small>
                                </div>
                                <h5 class="property-title card-title">{{ $property->title }}</h5>
                                ...
                                <div class="property-features d-flex gap-3 mb-3">
                                    <span class="small">
                                        <i class="fas fa-bed me-1"></i> {{ $property->bedrooms ?? '-' }} KT
                                    </span>
                                    <span class="small">
                                        <i class="fas fa-bath me-1"></i> {{ $property->bathrooms ?? '-' }} KM
                                    </span>
                                    <span class="small">
                                        <i class="fas fa-vector-square me-1"></i> {{ $property->area ?? '-' }} m²
                                    </span>
                                </div>
                                ...
                                <div class="mt-auto d-grid gap-2">
                                    ...
                                    @auth
                                        <a href="{{ route('properties.show', $property->id) }}#booking-section" class="btn btn-primary">
